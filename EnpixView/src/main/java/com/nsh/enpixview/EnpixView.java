...
public class EnpixView extends androidx.appcompat.widget.AppCompatImageView {
    ...
    Allocation allocationHeight;
    Allocation allocationWidth;
    ...
    public void decryptImage(Context context, Bitmap bitmap, String key, long duration) throws NoSuchAlgorithmException {
        this.context = context;
        setImageBitmap(decryptImg(bitmap, key, duration));
    }

    @RequiresApi(api = Build.VERSION_CODES.O)
    public void encryptImage(Context context, Bitmap bitmap, String key, long duration) throws NoSuchAlgorithmException {
        this.context = context;
        setImageBitmap(encryptImg(bitmap, key, duration));
    }

    @RequiresApi(api = Build.VERSION_CODES.O)
    public Bitmap getDecryptImage(Context context, Bitmap bitmap, String key, long duration) throws NoSuchAlgorithmException {
        this.context = context;
        return decryptImg(bitmap, key, duration);
    }

    @RequiresApi(api = Build.VERSION_CODES.O)
    public Bitmap getEncryptImage(Context context, Bitmap bitmap, String key, long duration) throws NoSuchAlgorithmException {
        this.context = context;
        return encryptImg(bitmap, key, duration);
    }

    @RequiresApi(api = Build.VERSION_CODES.O)
    private Bitmap encryptImg(Bitmap temp, String key, long duration) throws NoSuchAlgorithmException {

        mRS = RenderScript.create(context);
        int[] input = one_matrix(temp);
        int[] output = new int[input.length];

        Timestamp timestamp = new Timestamp(System.currentTimeMillis());
        duration = timestamp.getTime() / duration;
        int[] random_list = getList();
        ...
        for (int r : random_list) {
            if (r < column_key.length) column_key[c_index++] = r;
        }

        int counter = 0;
        for (int i = 0; i < len1; i += 16) {
            String sub_key = sha_key.substring(counter, counter + 16) + "1A0B95C4D37286EF";
            HashSet<Character> temp_set = new HashSet<>();
            int temp_index = 0;

            // fisher - yates algorithm
            for (int j = i; j < i + 16; j++) {

                // pick only unique 0-9 A-F values
                while (!temp_set.add(sub_key.charAt(temp_index))) temp_index++;

                // hex to decimal
                int index = Integer.parseInt(sub_key.charAt(temp_index) + "", 16);

                // swap
                int temp_col = column_key[j];
                column_key[j] = column_key[i + index];
                column_key[i + index] = temp_col;
            ...
            if (counter == 64) {
                counter = 0;
                sha_key = getSHA(sha_key).toUpperCase();
            }
        }

        ScriptC_column_shuffle cScript = new ScriptC_column_shuffle(mRS);

        allocationIn = Allocation.createSized(mRS, Element.I32(mRS), temp.getHeight() * temp.getWidth());
        allocationOut = Allocation.createSized(mRS, Element.I32(mRS), temp.getHeight() * temp.getWidth());
        allocationKey = Allocation.createSized(mRS, Element.I32(mRS), column_key.length);
        allocationIndex = Allocation.createSized(mRS, Element.I32(mRS), 1);
        allocationHeight = Allocation.createSized(mRS, Element.I32(mRS), 1);
        allocationWidth = Allocation.createSized(mRS, Element.I32(mRS), 1);

        allocationIn.copyFrom(input);
        allocationHeight.copyFrom(new int[]{temp.getHeight()});
        allocationWidth.copyFrom(new int[]{temp.getWidth()});
        allocationKey.copyFrom(column_key);
        allocationIndex.copyFrom(new int[]{0});

        cScript.bind_inimage(allocationIn);
        cScript.bind_outimage(allocationOut);
        cScript.bind_height(allocationHeight);
        cScript.bind_width(allocationWidth);
        cScript.bind_key(allocationKey);
        cScript.forEach_root(allocationIndex);
        ...
        for (int r : random_list) {
            if (r < row_key.length) row_key[r_index++] = r;
        }
        ...
        for (int i = 0; i < len1; i += 16) {
            ...
            for (int j = i; j < i + 16; j++) {
                ...
                // swap
                int temp_col = row_key[j];
                row_key[j] = row_key[i + index];
                row_key[i + index] = temp_col;
        ...
        ScriptC_row_shuffle rScript = new ScriptC_row_shuffle(mRS);

        allocationIn = Allocation.createSized(mRS, Element.I32(mRS), temp.getHeight() * temp.getWidth());
        allocationOut = Allocation.createSized(mRS, Element.I32(mRS), temp.getHeight() * temp.getWidth());
        allocationKey = Allocation.createSized(mRS, Element.I32(mRS), row_key.length);
        allocationIndex = Allocation.createSized(mRS, Element.I32(mRS), 1);
        allocationHeight = Allocation.createSized(mRS, Element.I32(mRS), 1);
        allocationWidth = Allocation.createSized(mRS, Element.I32(mRS), 1);

        allocationIn.copyFrom(output);
        allocationHeight.copyFrom(new int[]{temp.getHeight()});
        allocationWidth.copyFrom(new int[]{temp.getWidth()});
        allocationKey.copyFrom(row_key);
        allocationIndex.copyFrom(new int[]{0});

        rScript.bind_inimage(allocationIn);
        rScript.bind_outimage(allocationOut);
        rScript.bind_height(allocationHeight);
        rScript.bind_width(allocationWidth);
        rScript.bind_key(allocationKey);
        rScript.forEach_root(allocationIndex);

        allocationOut.copyTo(input);

        rScript.destroy();
        allocationIn.destroy();
        allocationOut.destroy();
        allocationHeight.destroy();
        allocationWidth.destroy();
        allocationKey.destroy();
        allocationIndex.destroy();
        mRS.destroy();

        Bitmap temp1 = Bitmap.createBitmap(temp.getWidth(), temp.getHeight(), Bitmap.Config.ARGB_8888);
        temp1.setPixels(input, 0, temp.getWidth(), 0, 0, temp.getWidth(), temp.getHeight());

        return temp1;
    }

    @RequiresApi(api = Build.VERSION_CODES.O)
    private Bitmap decryptImg(Bitmap pic1, String key, long duration) throws NoSuchAlgorithmException {

        mRS = RenderScript.create(context);
        int[] input = one_matrix(pic1);
        int[] output = new int[input.length];
        ...
        for (int i = 0; i < pic1.getHeight(); i++) row_key[i] = i;
        int r_index = 0;
        ...
        ScriptC_reverse_row_shuffle rScript = new ScriptC_reverse_row_shuffle(mRS);

        allocationIn = Allocation.createSized(mRS, Element.I32(mRS), pic1.getHeight() * pic1.getWidth());
        allocationOut = Allocation.createSized(mRS, Element.I32(mRS), pic1.getHeight() * pic1.getWidth());
        allocationKey = Allocation.createSized(mRS, Element.I32(mRS), row_key.length);
        allocationIndex = Allocation.createSized(mRS, Element.I32(mRS), 1);
        allocationHeight = Allocation.createSized(mRS, Element.I32(mRS), 1);
        allocationWidth = Allocation.createSized(mRS, Element.I32(mRS), 1);
        ...
        allocationIn.copyFrom(input);
        allocationHeight.copyFrom(new int[]{pic1.getHeight()});
        allocationWidth.copyFrom(new int[]{pic1.getWidth()});
        allocationKey.copyFrom(row_key);
        allocationIndex.copyFrom(new int[]{0});
        ...
        for (int i = 0; i < pic1.getWidth(); i++) column_key[i] = i;
        int c_index = 0;
        ...
        ScriptC_reverse_column_shuffle cScript = new ScriptC_reverse_column_shuffle(mRS);
        ...
        pic2.setPixels(input, 0, pic1.getWidth(), 0, 0, pic1.getWidth(), pic1.getHeight());
