...
exports.encrypt = function (image, key, duration) {
    ...
    var len1 = (Math.floor(image[0].length / 16) * 16)
    var column_key = []
    random_set.forEach(element => {
        if (element < image[0].length) column_key.push(element)
    });
    ...
    // key generation
    var base64 = crypto.createHash('sha256').update(key + duration).digest('hex').toUpperCase()
    var len1 = (Math.floor(image.length / 16) * 16)
    var row_key = []
    random_set.forEach(element => {
        if (element < image.length) row_key.push(element)
    });
